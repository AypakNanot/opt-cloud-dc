...
abstract class CompositeRuntimeTypeBuilder<S extends EffectiveStatement<?, ?>, R extends CompositeRuntimeType> {
    private static final Logger LOG = LoggerFactory.getLogger(CompositeRuntimeTypeBuilder.class);
        ...
        for (var stmt : statement.effectiveSubstatements()) {
            if (stmt instanceof SchemaTreeEffectiveStatement) {
                ...
                if (!isAugmentedChild(child.getIdentifier())) {
                    final var childGen = findChildGenerator(generator, child.argument().getLocalName());
                    if (childGen == null) {
                        LOG.error("Cannot find child for {} in {}", child, generator);
                    } else {
