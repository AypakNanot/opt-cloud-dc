...
public class NetconfDeviceCommunicator
    ...
    private static final Logger LOG_NETCONF_DEVICE_RPC = LoggerFactory.getLogger(NetconfDeviceRpc.class);
    ...
    private List<String> maskNotification= Stream.of("</netconf-session-end>","</netconf-session-start>").collect(Collectors.toList());
    /**
     * 最后一次接受netconf消息时间
     */
    private volatile long lastSucMsgTime = System.currentTimeMillis();
    ...
    public long getLastSucMsgTime() {
        return lastSucMsgTime;
    }
    ...
    public void onMessage(final NetconfClientSession session, final NetconfMessage message) {
         ...
         */
        lastSucMsgTime = System.currentTimeMillis();
        if (isNotification(message)) {
            if (LOG_NETCONF_DEVICE_RPC.isInfoEnabled()) {
                LOG_NETCONF_DEVICE_RPC.info("receive notification xml from device:{},rpc result:{},xml {}", getId(), true, message.toString());
            }
            if (ROOT_LOG.isInfoEnabled()) {
                ROOT_LOG.info("receive notification xml from device:{},rpc result:{},xml {}", getId(), true, message.toString());
            }
            if (!isMaskNotification(message)) {
    ...
    private boolean isMaskNotification(final NetconfMessage message) {
        if (Objects.isNull(message)) {
            return true;
        }
        String notification = message.toString();
        for (String maskStr : maskNotification) {
            if (notification.contains(maskStr)) {
                return true;
            }
        }
        return false;
    ...
    private static String msgToS(final NetconfMessage msg) {
        if(Objects.isNull(msg)||Objects.isNull(msg.getDocument())){
            LOG.error("NetconfMessage is null");
            return "";
        }
