...
public final class NetconfSessionPreferences {
    ...
    public boolean isCandidateSupported() {
        boolean isCandidateSupported = containsNonModuleCapability(NetconfMessageTransformUtil.NETCONF_CANDIDATE_URI.toString());
        if (isCandidateSupported) {
            // 根据 jvm 环境参数获取是否抑制 candidate 能力
            boolean inhibitionCandidate = Boolean.parseBoolean(System.getProperty("inhibition.capacity.candidate", "true"));
            if (inhibitionCandidate) {
                // 2024-06-20，对接华环OTN设备时，出现了删除业务无法 commit 成功，他们是在 NetconfDeviceDataBroker 的 newWriteOnlyTransaction() 始终改为 return WriteRunningTx
                // 这里进行处理，奥普泰设备不支持此项能力所以一直没有出现问题，但华环设备报了此项能力但设备在某些接口下依然有问题，所以此处直接进行屏蔽最为合适
                LOG.debug("optel modify this: when the device supports candidate, it still returns no support!!!");
                return false;
            }
            return true;
        }
