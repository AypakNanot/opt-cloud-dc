 ...
 */
package com.optel.tmaster.dc.device.impl.dci.cmcc.impl.common;
...
import org.opendaylight.yang.gen.v1.http.openconfig.net.yang.platform.rev230426.platform.component.top.ComponentsBuilder;
...
//            return ComponentsTestUtil.getAllComponents();
        }
    ...
    default Components getComponents(String neId, Set<String> nameList) {
        ...
        InstanceIdentifier<Components> root = create(Components.class);
        if (nameList.size() == 1) {
            //只有一条name数据作为主键
            KeyedInstanceIdentifier<Component, ComponentKey> iid =
                    root.child(Component.class, new ComponentKey(nameList.iterator().next()));
     ...
     * @return 过滤结果
     */
    default Map<ComponentKey, Component> filterByName(Map<ComponentKey, Component> dataMap, Set<String> nameList) {
        if (dataMap == null) {
            return null;
        }
        if (nameList == null || nameList.isEmpty()) {
