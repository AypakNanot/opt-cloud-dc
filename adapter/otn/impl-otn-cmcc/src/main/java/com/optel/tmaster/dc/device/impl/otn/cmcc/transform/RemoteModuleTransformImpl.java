 ...
 * date       time        author
 * ─────────────────────────────
 * 2021/10/11   10:17      liwenxue
 * Copyright (c) 2021, H-OPTEL All Rights Reserved.
 *
 * @author liwenxue
 * @version V1.0.0
 */
public class RemoteModuleTransformImpl extends AbstractCmccTransformer implements EnumTransform {

    public List<RemoteModule> devRemoteModuleToApiList(
            List<org.opendaylight.yang.gen.v1.com.optel.yang.otn.extension.rev200805.remote.module.grouping.RemoteModule> remoteModules) {
        if (remoteModules == null) {
            return CollUtil.newArrayList();
        }
        List<RemoteModule> result = new ArrayList<>();
        for (org.opendaylight.yang.gen.v1.com.optel.yang.otn.extension.rev200805.remote.module.grouping.RemoteModule remoteModule : remoteModules) {
            result.add(devRemoteModuleToApi(remoteModule));
        }
        return result;
    }

    public RemoteModule devRemoteModuleToApi(org.opendaylight.yang.gen.v1.com.optel.yang.otn.extension.rev200805.remote.module.grouping.RemoteModule remoteModule) {
