...
import com.optel.tmaster.dc.device.impl.otn.cmcc.transform.FtpTransformImpl;
...
import org.opendaylight.yang.gen.v1.com.optel.yang.otn.extension.rev200805.GfpfParameters;
import org.opendaylight.yang.gen.v1.com.optel.yang.otn.extension.rev200805.gfpf.parameter.grouping.GfpfParameter;
import org.opendaylight.yang.gen.v1.com.optel.yang.otn.extension.rev200805.gfpf.parameter.grouping.GfpfParameterKey;
import org.opendaylight.yang.gen.v1.urn.ccsa.yang.acc.devm.rev200708.Ftps;
import org.opendaylight.yang.gen.v1.urn.ccsa.yang.acc.devm.rev200708.ftps.Ftp;
import org.opendaylight.yang.gen.v1.urn.ccsa.yang.acc.devm.rev200708.ftps.FtpKey;
import org.opendaylight.yang.gen.v1.urn.ccsa.yang.acc.eos.rev190213.EthFtpPac1;
import org.opendaylight.yang.gen.v1.urn.ccsa.yang.acc.eth.rev200611.Ftp1;
 ...
 * @version V1.0.0
 */
public class CmccOptOtnFtpServiceImpl extends BaseOptOtnFtpServiceImpl implements IDeviceServiceOtnCmcc {
        ...
        if (NcTools.isSupportOptelExt(input.getNeId())) {
            ...
            if (ftpNameList == null) {
                ...
                InstanceIdentifier<GfpfParameter> iid =
                        create(GfpfParameters.class)
                                .child(GfpfParameter.class, new GfpfParameterKey(CollUtil.getFirst(ftpNameList)));
                ...
                if (gfpfParameters != null) {
                    outputBuilder.setGfpfParameter(
                            ltm(new FtpTransformImpl().devGfpfParameterToApiList(
                                    filterGfpfParameter(gfpfParameters.getGfpfParameter(), ftpNameList))));
                }
            }
        }
    ...
    private Map<GfpfParameterKey, GfpfParameter> filterGfpfParameter(Map<GfpfParameterKey, GfpfParameter> gfpfParameterList, Set<String> ftpNameList) {
    ...
    public ListenableFuture<RpcResult<ModifySdhFtpPacOutput>> modifySdhFtpPac(ModifySdhFtpPacInput input) {
        InstanceIdentifier<org.opendaylight.yang.gen.v1.urn.ccsa.yang.acc.sdh.rev200708.Ftp1> iid =
                create(Ftps.class).child(Ftp.class, new FtpKey(input.getFtpName()))
                        .augmentation(org.opendaylight.yang.gen.v1.urn.ccsa.yang.acc.sdh.rev200708.Ftp1.class);
        //下发数据到设备
