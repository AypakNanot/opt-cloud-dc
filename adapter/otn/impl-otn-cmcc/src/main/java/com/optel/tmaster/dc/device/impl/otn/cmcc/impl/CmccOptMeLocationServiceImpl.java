...
import com.optel.tmaster.dc.device.impl.otn.cmcc.util.OptelLocationUtil;
 ...
 **/
public class CmccOptMeLocationServiceImpl extends BaseOptMeLocationServiceImpl implements IDeviceServiceOtnCmcc {
    private final static Logger LOG = LoggerFactory.getLogger(OptMeLocationService.class);
    ...
    public ListenableFuture<RpcResult<GetPortStateOutput>> getPortState(GetPortStateInput input) {
        InstanceIdentifier<PortLightsState> iid = create(PortLightsState.class);
...
//        }
    ...
    public ListenableFuture<RpcResult<EqLocationsOutput>> eqLocations(EqLocationsInput input) {
        InstanceIdentifier<MeLocations> iid = create(MeLocations.class);
        MeLocations meLocations;

//        try {
//            meLocations = MountTools.queryFromOperational(input.getNeId(), iid);
//        } catch (Exception e) {
//            meLocations = OptelLocationUtil.getLocations(input.getDeviceType().intValue(), input.getProductName());
//        }
        meLocations = OptelLocationUtil.getLocations(input.getDeviceType().intValue(), input.getProductName());
