...
import com.optel.tmaster.dc.device.impl.otn.cmcc.transform.FileTransformImpl;
 ...
 * @version V1.0.0
 */
public class CmccOptOtnFileServiceImpl extends BaseOptOtnFileServiceImpl implements IDeviceServiceOtnCmcc {
    private static final Logger LOG = LoggerFactory.getLogger(CmccOptOtnFileServiceImpl.class);
    ...
    public ListenableFuture<RpcResult<DownloadFileNeOutput>> downloadFileNe(DownloadFileNeInput input) {
        AccFileService service = MountTools.getRpcService(input.getNeId(), AccFileService.class);
        ListenableFuture<RpcResult<DownloadFileOutput>> future = service.downloadFile(new FileTransformImpl().apiDownloadFileToDev(input).build());
        return RpcResultUtil.buildFutureResult(future);
    }
    ...
    public ListenableFuture<RpcResult<GetFileListNeOutput>> getFileListNe(GetFileListNeInput input) {
        ...
        return RpcResultUtil.buildFutureResult(resultFuture, result -> {
            ...
            if (files != null && files.getFile() != null) {
                return new FileTransformImpl().devGetFileListToApi(files.getFile().values()).build();
            }
