 ...
 * date       time        author
 * ─────────────────────────────
 * 2021/9/30   10:56      liwenxue
 * Copyright (c) 2021, H-OPTEL All Rights Reserved.
 *
 * @author liwenxue
 * @version V1.0.0
 */
public class CtcOptDevProtectionGroupServiceImpl extends BaseOptDevProtectionGroupServiceImpl implements IDeviceServiceOtnCtc {
    /**
     * 当前服务支持的Yang Mode类型
     *
     * @return 支持的类型
        ...
        if (NcTools.isSupportOptelExt(input.getNeId())) {
            ...
            if (pgIdList == null) {
                ...
                if (sfPersistencyTimers != null) {
                    outputBuilder.setSfPersistencyTimer(
                            ltm(new DevProtectionGroupTransformImpl().devSfPersistencyTimerToApi(
                                    filterSfPersistencyTimer(sfPersistencyTimers.getSfPersistencyTimer(), CollUtil.newArrayList(pgIdList)))));
                }
            }
        }
     ...
     * @return 过滤后的数据
     */
    private List<SfPersistencyTimer> filterSfPersistencyTimer(Map<SfPersistencyTimerKey, SfPersistencyTimer> sfPersistencyTimerList, List<Uint16> pgIdList) {
    ...
    public ListenableFuture<RpcResult<QueryDevProtectionGroupOutput>> queryDevProtectionGroup(QueryDevProtectionGroupInput input) {
        List<org.opendaylight.yang.gen.v1.urn.ccsa.yang.acc.protection.group.rev240702.eq.pgs.EqPg> pgs = new ArrayList<>();
        if (input.getDevPgId() != null) {
            InstanceIdentifier<org.opendaylight.yang.gen.v1.urn.ccsa.yang.acc.protection.group.rev240702.eq.pgs.EqPg> iid =
                    create(EqPgs.class).child(org.opendaylight.yang.gen.v1.urn.ccsa.yang.acc.protection.group.rev240702.eq.pgs.EqPg.class, new EqPgKey(input.getDevPgId()));
            org.opendaylight.yang.gen.v1.urn.ccsa.yang.acc.protection.group.rev240702.eq.pgs.EqPg eqPg = MountTools.queryFromOperational(input.getNeId(), iid);
            if (eqPg != null) {
                pgs.add(eqPg);
            }
        } else {
