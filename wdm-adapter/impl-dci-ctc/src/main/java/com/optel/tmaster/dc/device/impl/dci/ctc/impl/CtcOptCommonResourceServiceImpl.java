 ...
 * @version V1.0.0
 */
public class CtcOptCommonResourceServiceImpl extends BaseDciCommonResourceServiceImpl implements IDeviceServiceWdmCtc {
    ...
    public ListenableFuture<RpcResult<UpdateCommonResourceOutput>> updateCommonResource(UpdateCommonResourceInput input) {
        ...
        if (input.getPowerAdminState() == null) {
            if (uninstall) {
                ...
                @NonNull InstanceIdentifier<Config> iid = create(Components.class).child(Component.class, new ComponentKey(input.getName())).child(Config.class);
            ...
            // 上下电
            @NonNull InstanceIdentifier<Component1> iid = create(Components.class).child(Component.class, new ComponentKey(input.getName())).augmentation(Component1.class);
    ...
    public ListenableFuture<RpcResult<DeleteCommonResourceOutput>> deleteCommonResource(DeleteCommonResourceInput input) {
        @NonNull KeyedInstanceIdentifier<Component, ComponentKey> child = create(Components.class).child(Component.class, new ComponentKey(input.getName()));
