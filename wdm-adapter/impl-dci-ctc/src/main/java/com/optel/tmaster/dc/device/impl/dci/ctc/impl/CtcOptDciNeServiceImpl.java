...
import com.optel.tmaster.dc.device.impl.dci.ctc.transform.config.DciNeTransform;
...
import org.opendaylight.yang.gen.v1.com.optel.dci.yang.platform.types.rev220208.CHASSIS;
...
import org.opendaylight.yang.gen.v1.http.openconfig.net.yang.platform.rev200630.platform.anchors.top.chassis.State;
 ...
 * @version V1.0.0
 */
public class CtcOptDciNeServiceImpl extends BaseDciNeServiceImpl implements IDeviceServiceWdmCtc {
    ...
    public ListenableFuture<RpcResult<GetNeInfoOutput>> getNeInfo(GetNeInfoInput input) {
        ...
        Ntp ntp = MountTools.queryFromOperational(input.getNeId(), ntpIid);
        //lldp
        @NonNull InstanceIdentifier<Lldp> lldpIid = create(Lldp.class);
        Lldp lldp = MountTools.queryFromOperational(input.getNeId(), lldpIid);
        //设备机框上的数据
        Components result = filter(input.getNeId(), null, Collections.singleton(CHASSIS.class));
    ...
    public ListenableFuture<RpcResult<RebootOutput>> reboot(RebootInput input) {
        ...
        ListenableFuture<RpcResult<org.opendaylight.yang.gen.v1.http.openconfig.net.yang.rpc.rev200630.RebootOutput>> reboot = rpcService.reboot(dciNeTransform.apiToRebootInputDev(input));
    ...
    public ListenableFuture<RpcResult<SetDatetimeOutput>> setDatetime(SetDatetimeInput input) {
        OpenconfigRpcService rpcService = MountTools.getRpcService(input.getNeId(), OpenconfigRpcService.class);
        DciNeTransform dciNeTransform = new DciNeTransform();
        ListenableFuture<RpcResult<org.opendaylight.yang.gen.v1.http.openconfig.net.yang.rpc.rev200630.SetDatetimeOutput>> rpcResultListenableFuture = rpcService.setDatetime(dciNeTransform.apiToSetDatetimeInputDev(input));
