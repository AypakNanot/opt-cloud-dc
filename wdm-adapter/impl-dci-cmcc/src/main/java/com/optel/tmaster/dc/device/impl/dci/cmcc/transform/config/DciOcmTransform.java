...
import org.opendaylight.yang.gen.v1.http.openconfig.net.yang.channel.monitor.rev230426.channel.monitor.top.channel.monitors.channel.monitor.ConfigBuilder;
...
public class DciOcmTransform implements PortAndTransceiverAndOtdrAndOpticalChannelTypeTransform, PlatformTypeTransform {
    public GetOcmConfigOutput devToGetOchConfigOutputApi(ChannelMonitors monitors, Set<String> names) {
        ...
        for (org.opendaylight.yang.gen.v1.http.openconfig.net.yang.channel.monitor.rev230426.channel.monitor.top.channel.monitors.ChannelMonitor monitor : monitors.getChannelMonitor().values()) {
            ...
            if (monitor.getChannels() != null && monitor.getChannels().getChannel().size() != 0) {
                ...
                for (org.opendaylight.yang.gen.v1.http.openconfig.net.yang.channel.monitor.rev230426.media.channel.spectrum.power.top.channels.Channel channel : monitor.getChannels().getChannel().values()) {
    ...
    public Config apiToConfigDev(SetOcmConfigInput input){
        ...
        configBuilder.setLowerFrequency(new org.opendaylight.yang.gen.v1.http.openconfig.net.yang.transport.types.rev230426.FrequencyType(input.getLowerFrequency().getValue()));
        configBuilder.setMonitorPort(input.getMonitorPort());
        configBuilder.setName(input.getName());
        configBuilder.setUpperFrequency(new org.opendaylight.yang.gen.v1.http.openconfig.net.yang.transport.types.rev230426.FrequencyType(input.getUpperFrequency().getValue()));
