    ...
    private final LoadingCache<@NonNull QName, @NonNull Class<? extends BaseIdentity>> identityClasses =
        CacheBuilder.newBuilder().weakValues().build(new CacheLoader<>() {
            ...
            public Class<? extends BaseIdentity> load(final QName key) {
                QName qName = key;
                if ("urn:ietf:params:xml:ns:yang:iana-if-type".equals(key.getNamespace().toString())) {
                    qName = QName.create(QNameModule.create(XMLNamespace.of("urn:ietf:params:xml:ns:yang:iana-if-type"), Revision.of("2021-06-21")), key.getLocalName());
                }
                final var type = getTypes().findIdentity(qName).orElseThrow(
