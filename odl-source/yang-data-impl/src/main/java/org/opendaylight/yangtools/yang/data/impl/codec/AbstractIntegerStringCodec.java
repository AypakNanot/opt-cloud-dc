...
public abstract class AbstractIntegerStringCodec<N extends Number & Comparable<N>,
    ...
    private static final Logger LOG = LoggerFactory.getLogger(AbstractIntegerStringCodec.class);
    ...
    protected final N deserializeImpl(final String product) {
        ...
        //dc兼容处理  空标签的int类型处理为0
        if ("".equals(stringRepresentation)) {
            deserialized = verifyNotNull(deserialize("0", base));
        } else {
            deserialized = verifyNotNull(deserialize(stringRepresentation, base));
        }
        if (rangeConstraint != null) {
            final RangeSet<N> ranges = rangeConstraint.getAllowedRanges();
            if (!ranges.contains(deserialized)) {
                //dc兼容处理  int类型超出范围抛错
                LOG.error("Value '" + deserialized + "'  is not in required ranges " + ranges, new YangInvalidValueException(ErrorType.APPLICATION, rangeConstraint,
    ...
    private static int provideBase(final String integer) {
        if (integer.length() == 1 && integer.charAt(0) == '0' || INT_PATTERN.matcher(integer).matches()) {
            ...
            //dc兼容处理
            LOG.error("Incorrect lexical representation of integer value: " + integer + ".\n"
                    + "An integer value can be defined as:\n"
                    + "  - a decimal number,\n"
                    + "  - a hexadecimal number (prefix 0x)," + "%n  - an octal number (prefix 0).\n"
                    + "Signed values are allowed. Spaces between digits are NOT allowed.", new NumberFormatException());
