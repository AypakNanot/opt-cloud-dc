...
public class StringStringCodec extends TypeDefinitionAwareCodec<String, StringTypeDefinition>
    ...
    private static final Logger LOG = LoggerFactory.getLogger(StringStringCodec.class);
    ...
    void validate(final String str) {
        if (lengthConstraint != null) {
            ...
            if (!ranges.contains(str.codePointCount(0, str.length()))) {
                //dc兼容处理 字符串不符合长度规范
                LOG.error("String " + str + " does not match allowed lengths " + ranges, new YangInvalidValueException(ErrorType.APPLICATION, lengthConstraint,
