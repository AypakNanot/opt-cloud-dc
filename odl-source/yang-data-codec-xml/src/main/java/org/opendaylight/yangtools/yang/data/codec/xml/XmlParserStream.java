        ...
        switch (in.nextTag()) {
            case XMLStreamConstants.START_ELEMENT:
                ...
                while (in.hasNext()) {
                    ...
                    String parenUri = parentSchema.getQName().getNamespace().toString();
                    /*
                     * 2022.05.23
                     * 因为波分设备v1.0的rpc上报时的命名空间问题，
                     * 需要对性能模块的get-pm-data和rpc模块的get-oduk-delay进行特殊处理
                     * FR：46619、44558
                     * by lxy
                     */
                    if (parenUri.equals("http://openconfig.net/yang/performance") || parenUri.equals("http://openconfig.net/yang/rpc")) {
                        if ("urn:ietf:params:xml:ns:netconf:base:1.0".equals(elementNS) || "http://openconfig.net/yang/rpc".equals(elementNS)) {
                            elementNS = parenUri;
                        }
                    }
                    // II型机 兼容supported-layer-protocol-names未正确指定域名空间
                    if ("supported-layer-protocol-names".equals(xmlElementName) && "http://openconfig.net/yang/platform".equals(elementNS)) {
                        elementNS = "http://openconfig.net/yang/platform/port";
                    }
                    ...
                    if (strictParsing) {
                        //dc兼容处理 找不到节点错误
                        LOG.error("Schema for node with name {} and namespace {} does not exist in parent {}", xmlElementName,
