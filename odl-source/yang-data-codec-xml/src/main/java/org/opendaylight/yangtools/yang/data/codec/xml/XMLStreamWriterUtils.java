...
abstract class XMLStreamWriterUtils {
    ...
    /**
     * 根据命名空间获取yang文件中声明的prefix.
     *
     * @param nameSpace 命名空间。
     * @return prefix。当未找到时，返回null
     */
    private String getYangPrefix(String nameSpace) {
        if (nameSpace == null || "".equals(nameSpace)) {
            return null;
        }
        if (this instanceof SchemaAwareXMLStreamWriterUtils) {
            if (((SchemaAwareXMLStreamWriterUtils) this)
                    .modelContext() == null) {
                return null;
            }
            // 从schema中获取prefix
            Collection<? extends Module> modules = ((SchemaAwareXMLStreamWriterUtils) this)
                    .modelContext().getModules();
            if (modules != null) {
                for (Module module : modules) {
                    if (nameSpace.equals(module.getQNameModule().getNamespace().toString())) {
    ...
    @VisibleForTesting
    String encode(final @NonNull ValueWriter writer, final @NonNull QName qname, final QNameModule parent)
        ...
        var prefix = "x";
        // 将prefix使用yang文件中声明的prefix代替。解决FR0049779，DCI设备不识别“x”前缀问题
        String getPrefix = getYangPrefix(ns);
    ...
    private String encode(final @NonNull ValueWriter writer, final @NonNull IdentityrefTypeDefinition type,
            final @NonNull Object value, final QNameModule parent) throws XMLStreamException {
