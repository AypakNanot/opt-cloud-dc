...
import org.opendaylight.yang.gen.v1.com.optel.params.yang.snmp.config.rev211113.SnmpGetOutputBuilder;
...
import org.opendaylight.yang.gen.v1.com.optel.params.yang.snmp.config.rev211113.snmp.get.output.ResultBuilder;
 ...
 * date       time        author
 * ─────────────────────────────
 * 2021/11/30   10:29      liwenxue
 * Copyright (c) 2021, H-OPTEL All Rights Reserved.
 *
 * @author liwenxue
 * @version V1.0.0
 */
public class SnmpAsyncGetHandler implements ResponseListener {
    private static final Logger LOG = LoggerFactory.getLogger(SnmpAsyncGetHandler.class);
    ...
    private final List<Result> resultsArrayList = new ArrayList<>();
    private Target target;
    private PDU pdu;
    private OID oid;
    private Snmp snmp;

    public SnmpAsyncGetHandler(SnmpGetInput getInput, Snmp snmp) {
        snmpGetInput = getInput;
        this.snmp = snmp;
        pdu = new PDU();
        oid = new OID(snmpGetInput.getOid());
        pdu.add(new VariableBinding(oid));
        pdu.setMaxRepetitions(SnmpRequestUtil.MAXREPETITIONS);
        pdu.setNonRepeaters(0);
        target = SnmpRequestUtil.getTargetForIp(getInput);
    ...
    private void storeResult(VariableBinding variableBinding) {
        ...
        result.setOid(oidString);
        result.setValue(val);
        resultsArrayList.add(result.build());
    }
    ...
    private void setResult(Throwable e) {
        ...
        getOutputBuilder.setResult(resultsArrayList);
    ...
    public SettableFuture<RpcResult<SnmpGetOutput>> getRpcResponse() {
        ...
        try {
            ...
            errorOutput.withError(ErrorType.APPLICATION, "IOException when sending GET request");
